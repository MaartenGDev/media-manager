...
var MediaManager = function () {
  ...
  _createClass(MediaManager, [{
    ...
    value: function _buildHeader() {
      ...
      var text = document.createTextNode(settings.labels.title);
    ...
    value: function _buildResourcePreviews() {
      ...
      var resources = settings.source.resources.map(function (resource) {
        ...
        deleteIcon = (0, _css.addClassesToNode)(deleteIcon, settings.classes.deleteItemIcon);
    ...
    value: function _toggleMediaManager() {
      ...
      var mediaManager = settings.elements.wrapper.querySelector((0, _css.createSelector)(settings.classes.wrapper));
    ...
    key: '_clearSelectedItems',
    value: function _clearSelectedItems() {
      var classes = this.settings.classes;

      var activeItemsSelector = (0, _css.createSelector)([classes.item, classes.activeItem]);

      console.log(activeItemsSelector);
      document.querySelectorAll(activeItemsSelector).forEach(function (x) {
        x.classList.remove(classes.activeItem);
      });
    }
  }, {
    key: '_appendMediaManagerToWrapper',
    value: function _appendMediaManagerToWrapper() {
    ...
    value: function _registerEventListenersForMediaActions() {
      ...
      settings.elements.wrapper.addEventListener('click', function (_ref) {
        ...
        if (target.classList.contains(settings.classes.item)) {
          var selectedResources = _this.settings.state.selectedResources;
          ...
          if (hasBeenSelected) {
            _this.settings.state.selectedResources = selectedResources.filter(function (resource) {
              return resource.path !== path;
            });

            target.classList.toggle(settings.classes.activeItem, false);
          } else if (!hasSelectedMaxItems) {
            _this.settings.state.selectedResources = [].concat(_toConsumableArray(selectedResources), [settings.source.resources.find(function (resource) {
              return resource.path === path;
            })]);
      ...
      document.querySelector(confirmSelector).addEventListener('click', function () {
        settings.events.onConfirm(_this.settings.state.selectedResources);
    ...
    key: '_registerDeleteItemEventListeners',
    value: function _registerDeleteItemEventListeners() {
      var _this2 = this;
      ...
      var deleteItemSelector = (0, _css.createSelector)(classes.deleteItemIcon);

      [].concat(_toConsumableArray(document.querySelectorAll(deleteItemSelector))).forEach(function (e) {
        e.addEventListener('click', function (evt) {
          evt.preventDefault();
          var selectedResource = source.resources.find(function (x) {
            return x.path === evt.target.dataset.src;
          });
          events.onDeleteItem(selectedResource).then(function (deleteIsConfirmed) {
            if (deleteIsConfirmed) {
              _this2._remove(selectedResource);
            }
          });
        });
      });
    ...
    value: function _remove(resource) {
      this.settings.source.resources = [].concat(_toConsumableArray(this.settings.source.resources)).filter(function (x) {
        return x.path !== resource.path;
      });
      this.settings.state.selectedResources = [];
    ...
    key: '_refreshResourcePreviews',
    value: function _refreshResourcePreviews() {
